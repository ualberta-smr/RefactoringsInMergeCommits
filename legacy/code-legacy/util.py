...
def get_merge_data(project_name, merge_commit_hash):
    ...
    merge_output = os.popen('cd ./../projects/' + project_name + ';' +
                            ...
                            'git merge ' + ' '.join(merge_parents[1:])).read().rstrip()
    java_conflicts = list()
    ...
    for output_line in merge_output.split('\n'):
        if output_line.startswith('CONFLICT'):
            ...
            if path.lower().endswith('.java'):
                java_conflicts.append((conflict_type, path, get_java_file_package(project_name, path)))

    is_conflicting = 'automatic merge failed' in merge_output.lower()
    os.popen('cd ./../projects/' + project_name + '; git reset --hard')
    return is_conflicting, java_conflicts
...
def parse_refactoring_details(refactoring_type, refactoring_details):
    ...
    if refactoring_type in binary_patterns:
        ...
        if refactoring_type == 'Extract Interface' or refactoring_type == 'Extract Superclass':
            destination_class, source_class = source_class, destination_class
