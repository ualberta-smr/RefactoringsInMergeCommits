...
-- Tue 24 Jul 2018 04:36:59 PM MDT
...
CREATE 
    ...
    DEFINER = `root`@`localhost` 
    SQL SECURITY DEFINER
VIEW `all_refactorings` AS
    SELECT 
        `project`.`name` AS `project_name`,
        `project`.`url` AS `project_url`,
        `merge_commit`.`commit_hash` AS `merge_commit_hash`,
        `merge_parent`.`commit_hash` AS `merge_parent_hash`,
        `refactoring_commit`.`commit_hash` AS `refactoring_commit_hash`,
        `refactoring_commit`.`refactoring_type` AS `refactoring_type`,
        `refactoring_commit`.`refactoring_detail` AS `refactoring_detail`
    FROM
        (((`merge_commit`
        JOIN `project` ON ((`project`.`id` = `merge_commit`.`project`)))
        JOIN `merge_parent` ON ((`merge_commit`.`commit_hash` = `merge_parent`.`merge_commit`)))
        LEFT JOIN `refactoring_commit` ON ((`merge_parent`.`id` = `refactoring_commit`.`merge_parent`)))
    ORDER BY `project`.`name` , `merge_commit`.`commit_hash` , `merge_parent`.`id` , `refactoring_commit`.`id` , `refactoring_commit`.`refactoring_type`;
